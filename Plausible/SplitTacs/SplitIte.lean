...
-/
private def andThenOnSubgoals (tac1 : TacticM Unit) (tac2 : TacticM Unit) : TacticM Unit :=
  focus do
    tac1
...
private partial def splitIfsCore
  ...
  if no_split then
    (reduceIfsAt loc)
    (splitIfsCore loc hNames (cond::done) <|> pure ())
  else do
    let hName ← getNextName hNames
    (splitIf1 cond hName loc)
    (splitIfsCore loc hNames (cond::done)) <|>
    pure ()
...
-/
syntax (name := _splitIfs) "_split_ifs" (location)? (" with" (ppSpace colGt binderIdent)+)? : tactic

elab_rules : tactic
| `(tactic| _split_ifs $[$loc:location]? $[with $withArg*]?) =>
